        ...
        <>
            <BaseControl>
                <RangeControl
                    ...
                    onChange = { setValue }
                    value={size && parseInt(size[tabName]) || 48}
                    min={ 0 }
                    max={ 200 }
                    currentInput = { 48 }
                    allowReset = { true }
...
const itemWidthClassGenerator = (itemWidth, includePrefix = true) => {
    const cssVariables = [];
    ...
    for (const breakpoint in itemWidth) {
        ...
        if (widthValue) {
            const variableName = includePrefix ? `--width${breakpointCss}` : `${breakpointCss}`;
            if (includePrefix) {
