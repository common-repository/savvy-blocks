...
function IconSelector ( props ) {
    ...
    // const savvyBlocksUrl = savvyParams.savvyBlocksUrl;
    ...
    useEffect( () => {
        apiFetch( { path: `/savvy/v1/icon-svg/list` } )
            .then (
                ( data ) => {
                    setIcons( data )
                }
    ...
    icons.map(icon => {
        IconItems.push(
            <span key={icon.ID} className={ icon.ID === selectedIcon ? 'icon-item is-selected' : 'icon-item' } onClick={ () => setAttributes({ icon: icon.ID }) }>
                <img
                    src={icon.guid}
                />
    ...
    const upload = ( files ) => {
        mediaUpload( {
            ...
            filesList: files,
            onFileChange: ( images ) => {
                apiFetch( { path: addQueryArgs( `/savvy/v1/icon-svg/add`, {id: images[0].id} ) } )
                    .then (
                        ( data ) => {
                            apiFetch( { path: `/savvy/v1/icon-svg/list` } )
                                .then (
    ...
    const removeIcon = (id) => {
        apiFetch( { path: addQueryArgs( `/savvy/v1/icon-svg/remove`, {id: id} ) } )
            .then (
                ( data ) => {
                    apiFetch( { path: `/savvy/v1/icon-svg/list` } )
                        .then (
